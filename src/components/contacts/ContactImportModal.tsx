  ...
  const prepareDataForImport = (): Partial<Contact>[] => {
    return parsedData.map(row => {
      ...
      const contact: Partial<Contact> = {
        ...
        email: undefined,
        phone: undefined,
        city: row[fieldMapping.city] ? sanitize.whitespace(row[fieldMapping.city]) : undefined,
        state: row[fieldMapping.state] ? sanitize.whitespace(row[fieldMapping.state]).toUpperCase() : undefined,
        specialty: row[fieldMapping.specialty] ? sanitize.whitespace(row[fieldMapping.specialty]) : undefined,
        type: 'imported',
        overall_score: 50,
        is_starred: false,
        tags: [],
