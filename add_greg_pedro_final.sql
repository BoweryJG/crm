-- Add Dr. Greg Pedro to CRM - Final version using correct tables
...
-- Add Dr. Greg Pedro to contacts table
...
-- Add Cindi Weiss to contacts table
INSERT INTO contacts (
    id,
    first_name,
    last_name,
...
-- Also add to public_contacts for call_analysis foreign key
INSERT INTO public_contacts (
    ...
    'a1b2c3d4-e5f6-7890-abcd-ef1234567890',
    'Dr. Greg Pedro MD',
    'Decision maker for practice technology and marketing. Practice at 4300 Hylan Blvd, Staten Island, NY. Website: gregpedromd.com',
    (SELECT id FROM auth.users LIMIT 1),
    (SELECT id FROM auth.users LIMIT 1)
) ON CONFLICT (id) DO NOTHING;

-- Create call analysis record (now it should work with public_contacts)
...
-- Create sales activity (removing practice_id since it doesn't exist in this table)
...
UNION ALL
SELECT 'Contact Added (contacts):', first_name || ' ' || last_name || ' - ' || email FROM contacts WHERE id = 'b2c3d4e5-f6a7-8901-bcde-f23456789012'
UNION ALL
SELECT 'Contact Added (public_contacts):', first_name || ' ' || last_name || ' - ' || email FROM public_contacts WHERE id = 'b2c3d4e5-f6a7-8901-bcde-f23456789012'
UNION ALL
